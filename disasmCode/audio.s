...
_channelCmdf0:
	...
	ld hl,wChannelDutyCycles
	call _getChannelVarInHL
	ld (hl),a		;puts second byte into wChannelDutyCycles using only bits 0-5 of input byte
	ld a,$41
	ld hl,wc039
	call _getChannelVarInHL
...
_isChannel7:
	call _getNextChannelByte
	ld ($ff00+R_NR42),a		;unmodified input byte put here
	ld a,$00
	ld ($ff00+R_NR41),a		;$00 here
	ld a,$80
	ld (wc01c),a			;$80 here
	jp _doNextChannelCommand
...
; for Channel 4/5 the Wave channel
@wave:
	call _getNextChannelByte
	ld hl,wChannelDutyCycles
	call _getChannelVarInHL
...
;	cp $00
	or a
	jr z,++		;allows channel volume to update if wChannelsEnabled is $00
@ret:
	ret
++
	ld a,(wSoundChannel)
...
;;	cp $00
	or a
	jr z,+			;no jump if cmdf0 used
	ld ($ff00+R_NR44),a		;writes wc01c to here
+
	ld a,$00
	ld (wc01c),a	;resets wc01c, making cmdf0 single-use for Channel 7
	jp _setChannelWaitCounter

;;
; enables channel, namely allowing volumes to update
_channelCmdff:
	ld a,$00
	ld hl,wChannelsEnabled
call _getChannelVarInHL
