...
4. Use ".redefine BEAT [value] if you would like to take out a common factor of your input lengths.
	a. For example, if you have
		beat a 12 b 24 c 6
	b. You can shorten this by using
		.redefine BEAT 6
		beat a 2 b 4 c 1
	c. NOTE: This does not change the size of byte (in-game data will still read 12, 24, etc. [$0c, $18, etc.])
...
7. Use ".redefine NOTE_END_WAIT [value]" if you wish to shorten notes using rests in between.
	a. You can also change the volume
		vol $6
		beat a 12
		vol $3
		beat a 12
		vol $6
		beat b 6 d 6 c 12
	b. You can also change the envelope
		env $0 $04
		beat a 24
		env $0 $00
		beat b 6 d 6 c 12
...
3. The Beat macro can use either relative or absolute systems of pitches.
	a. Absolute system: Pitches have notation of octave
		i. d5 c3 g4
	b. Relative system: Pitches do not have notation of octave
		i. d c g
		ii. Go to Pitches section to learn how to set octaves in the Relative system.
	c. Thus, the Beat macro can either look like this
...
1. Always start music with the Octave macro, as that will define your octave when using relative pitches.
	a. For example, if the base game data shows
		note g3 $18
	b. This translates as
		octave 3
		beat g 24
2. You can shift octaves by either using the Beat macro
	a. beat ou		;for octave up
	b. beat od		;for octave down
	c. These can be combined into single lines with actual pitch data
		i. beat a 8 b 8 ou c 8 d 24
...
1. When creating labels, notate:
	a. the name of the piece					[mapleMinigame]
	b. the name of the channel 					[Channel1]
	c. the measure of the label or pertinant info about the label	[Measure2]
	d. Example:							mapleMinigameChannel1Measure2
2. Jumps, named the Goto macro, are used by the base game to repeat a whole piece. There is only one input:
	a. place/label to jump
	b. Example:							goto mapleMinigameChannel1Measure2
3. Conditional jumps are used by all of these custom pieces and thus three files for your hack must be changed.
	...
	c. include/wram.s
	d. I include in this repository the updated versions of what you'll need. Hopefully it is compatible.
4. The conditional jump, named the GotoCond macro, has two inputs:
	a. required coda value
	b. place/label to jump
	c. Example:			 				gotoCond 2 mapleMinigameChannel1Measure2


Coda Macros
---------------------------------
1. There are two macros that can change the coda (value that is used by the conditional jump to know what to do).
	a. resetCoda		(sets coda to 0 [$00])
	b. incCoda		(increases coda by 1 [$01])
...
4. There are two main ways to use the Coda macros with the conditional jump:
	a. If you want to only repeat once:
	...
	b. If you want to repeat more than once:
...
5. incCoda can be stacked, meaning that a ResetCoda is not necessary to create a new chain.
			a. You should notate the value of coda after noteworthy jumps so to not get confused.
			b. Example:
